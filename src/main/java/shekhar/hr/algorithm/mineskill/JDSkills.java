package shekhar.hr.algorithm.mineskill;

import com.comcast.ebi.common.fileutils.UtilitiesFile;
import com.comcast.ebi.common.tokenizerutils.TokenizerActionSelector;
import com.comcast.ebi.common.tokenizerutils.UtilitiesTokenizer;
...
public class JDSkills {
    ...
    public static void main(String[] args) {
        JDSkills jdSkills = new JDSkills();

        String jd = jdSkills.getJD().toLowerCase();
        String jdClean = jd.replaceAll("\\."," ").replaceAll("\n"," ").replaceAll(","," ").replaceAll("\\\\", " ").replaceAll("/", " ");

        for (int i=4; i>0; --i) {
            List<String> jdtokens = jdSkills.getNgram(jdClean, i);
            jdClean = jdSkills.cleanText(jdtokens, jdClean);
